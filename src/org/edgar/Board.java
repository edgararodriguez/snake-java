...
//class  board displays and manages the game board.
public class Board extends JPanel{
	...
	//the size of each title in pixels 
	...
	//The length of the eyes from the base 
	...
	private SnakeGameplay game;
	
	//the array of tiles that make up this game
	private TileType[] tiles;
	
	//Creates a new BoardPanel instance.
	public Board(SnakeGameplay game) {
		this.game = game;
	...
	public void paintComponent(Graphics g) {
		...
		//shows message on the screen of the game progress
		if(game.isGameOver() || game.isNewGame() || game.isGamePaused()) {
			g.setColor(Color.WHITE);
			
			//gets the center coordinate of the game 
			int centerX = getWidth() / 2;
			int centerY = getHeight() / 2;
			
			//Shoots message based on the state of the game
			...
			//sets the message in the center of the game
			g.setFont(FONT);
	...
	//Runs through a switch statement because the titles are different

		switch(type) {
		
		//The fruit is drawn as a small red circle
		case Fruit:
			g.setColor(Color.RED);
			g.fillOval(x + 2, y + 2, TILE_SIZE - 4, TILE_SIZE - 4);
			break;
			
		//The snake body is drawn to fit a whole tile 
		case SnakeBody:
			g.setColor(Color.GREEN);
			g.fillRect(x, y, TILE_SIZE, TILE_SIZE);
			break;
			
		//The head is similar to the body but with two lines to represent the eyes
		case SnakeHead:
